...
Процедура ЗагруженныеДокументыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ПоказатьЗначение(,Элементы.ЗагруженныеДокументы.ТекущиеДанные.Документ);
КонецПроцедуры

&НаКлиенте
Процедура ПроблемныеДокументыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ПоказатьЗначение(,Элементы.ПроблемныеДокументы.ТекущиеДанные.Документ);
КонецПроцедуры
...
Процедура ЗаполнитьДокументЗаказПоставщику(ОткрытаяБаза, ДокументПриемник, ДокументИсходный)
	...
	ДокументПриемник.ДатаПоступления = ДокументПриемник.Дата;
	...
	Для Каждого Строка Из ТабличнаяЧастьТовары Цикл
		// TODO Можно выделить в отдельную процедуру
		НоваяСтрокаТовары = ТабличнаяЧастьПриемник.Добавить();
		ЗаполнитьСтрокуТовары(НоваяСтрокаТовары, Строка);
		НоваяСтрокаТовары.Склад           = ДокументПриемник.Склад;
		НоваяСтрокаТовары.СтавкаНДС       = СтавкаНДС(ОткрытаяБаза, Строка);
		НоваяСтрокаТовары.ДатаПоступления = ДокументПриемник.Дата;
	КонецЦикла;
	
	ЗаписатьДокумент(ДокументПриемник, ДокументИсходный.Проведен);
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьДокументЗаказКлиента(ОткрытаяБаза, ДокументПриемник, ДокументИсходный)
	...
	ДокументПриемник.Организация     = Организация;
	ДокументПриемник.Договор         = Договор(ДокументПриемник.Контрагент, ДокументИсходный.ДоговорКонтрагента.Наименование);
	ДокументПриемник.Склад           = Справочники.Склады.НайтиПоНаименованию(ДокументИсходный.СкладГруппа.Наименование);
	ДокументПриемник.СуммаДокумента  = ДокументИсходный.СуммаДокумента;
	ДокументПриемник.Валюта          = Справочники.Валюты.НайтиПоКоду(ДокументИсходный.ВалютаДокумента.Код);
	ДокументПриемник.ДатаОтгрузки    = ДокументПриемник.Дата;
	ДокументПриемник.Статус = Перечисления.СтатусыЗаказовКлиентов.КОтгрузке;
	ДокументПриемник.ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоНакладным;
	...
	Для Каждого Строка Из ТабличнаяЧастьТовары Цикл
		НоваяСтрокаТовары = ТабличнаяЧастьПриемник.Добавить();
		НоваяСтрокаТовары.Склад              = ДокументПриемник.Склад;
		НоваяСтрокаТовары.ДатаОтгрузки       = ДокументПриемник.Дата;
		НоваяСтрокаТовары.СтавкаНДС          = СтавкаНДС(ОткрытаяБаза, Строка);
		НоваяСтрокаТовары.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Отгрузить;
		ЗаполнитьСтрокуТовары(НоваяСтрокаТовары, Строка);
	КонецЦикла;
...
&НаСервере
Процедура ЗаполнитьДокументРеализация(ОткрытаяБаза, ДокументПриемник, ДокументИсходный)
	...
	ДокументПриемник.СуммаДокумента  = ДокументИсходный.СуммаДокумента;
	ДокументПриемник.Валюта          = Справочники.Валюты.НайтиПоКоду(ДокументИсходный.ВалютаДокумента.Код);
	ДокументПриемник.Статус = Перечисления.СтатусыЗаказовКлиентов.КОбеспечению;
	ДокументПриемник.ЗаказКлиента  = ЗаказКлиента(ДокументПриемник.Контрагент, ДокументИсходный.Сделка);
	ДокументПриемник.РеализацияПоЗаказам  = ЗначениеЗаполнено(ДокументПриемник.ЗаказКлиента);
	
	ТабличнаяЧастьТовары = ДокументИсходный.Товары;
	ТабличнаяЧастьПриемник = ДокументПриемник.Товары;
	ТабличнаяЧастьПриемник.Очистить();
	НеправильныйЗаказ = Ложь;
	Если ЗначениеЗаполнено(ДокументПриемник.ЗаказКлиента) Тогда
		СуммаЗаказа = ДокументПриемник.ЗаказКлиента.СуммаДокумента;
		СуммаДокумента = ДокументПриемник.СуммаДокумента;
		НеправильныйЗаказ = (СуммаЗаказа <> СуммаДокумента);
		Документы.РеализацияТоваровУслуг.ЗаполнитьПоОстаткамЗаказов(
			ДокументПриемник, 
			ТабличнаяЧастьПриемник,
			ДокументПриемник.СкидкиНаценки, 
			ДокументПриемник.Склад, 
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДокументПриемник.ЗаказКлиента),
			Новый Структура);
	Иначе
		Для Каждого Строка Из ТабличнаяЧастьТовары Цикл
			НоваяСтрокаТовары = ТабличнаяЧастьПриемник.Добавить();
	...
	Если НеправильныйЗаказ Тогда
		ТекстОшибки = СтрШаблон("Сумма в %1 и в %2 отличаются. Проверьте правильность заполнения", ДокументПриемник.ЗаказКлиента, ДокументПриемник.Ссылка);
		ДобавитьДокументВПроблемы(ДокументПриемник, "Неправильный заказ", ТекстОшибки);
	КонецЕсли;
...
Процедура ЗаполнитьДокументПоступление(ОткрытаяБаза, ДокументПриемник, ДокументИсходный)
	...
	ДокументПриемник.СуммаДокумента  = ДокументИсходный.СуммаДокумента;
	ДокументПриемник.ЗаказПоставщику  = ЗаказПоставщику(ДокументПриемник.Контрагент, ДокументИсходный.Сделка);
	ДокументПриемник.ПоступлениеПоЗаказам  = ЗначениеЗаполнено(ДокументПриемник.ЗаказПоставщику);
	...
	Если ЗначениеЗаполнено(ДокументПриемник.ЗаказПоставщику) Тогда
		СуммаЗаказа = ДокументПриемник.ЗаказПоставщику.СуммаДокумента;
		СуммаДокумента = ДокументПриемник.СуммаДокумента;
		НеправильныйЗаказ = (СуммаЗаказа <> СуммаДокумента);
		Документы.ПоступлениеТоваровУслуг.ЗаполнитьПоОстаткамЗаказов(
			...
			ТабличнаяЧастьПриемник, 
			ДокументПриемник.Склад, 
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДокументПриемник.ЗаказПоставщику));
	Иначе
		Для Каждого Строка Из ТабличнаяЧастьТовары Цикл
			НоваяСтрокаТовары = ТабличнаяЧастьПриемник.Добавить();
			НоваяСтрокаТовары.Склад     = ДокументПриемник.Склад;
	...
	Если НеправильныйЗаказ Тогда
		ТекстОшибки = СтрШаблон("Сумма в %1 и в %2 отличаются. Проверьте правильность заполнения", ДокументПриемник.ЗаказПоставщику, ДокументПриемник.Ссылка);
		ДобавитьДокументВПроблемы(ДокументПриемник, "Неправильный заказ", ТекстОшибки);
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьСтрокуТовары(НоваяСтрока, Строка)
	
	НоваяСтрока.Номенклатура       = Справочники.Номенклатура.НайтиПоКоду(Строка.Номенклатура.Код);
	НоваяСтрока.Характеристика     = Характеристика(НоваяСтрока.Номенклатура, Строка.ХарактеристикаНоменклатуры.Наименование);
	НоваяСтрока.КоличествоУпаковок = Строка.Количество;
	НоваяСтрока.Количество         = Строка.Количество;
	НоваяСтрока.Цена               = Строка.Цена;
	НоваяСтрока.Сумма              = Строка.Сумма;
	НоваяСтрока.СуммаНДС           = Строка.СуммаНДС;
	НоваяСтрока.СуммаСНДС          = Строка.Сумма + Строка.СуммаНДС; // TODO Здесь может быть неправильно
	
КонецПроцедуры

&НаСервере
Процедура ЗаписатьДокумент(Документ, Провести)
	...
	Исключение
		...
		ДобавитьДокументВПроблемы(Документ, "Не провелся", ОписаниеОшибки());
	КонецПопытки;
...
Процедура ЗагрузитьДокументыНаСервере()
	...
	ПроблемныеДокументы.Очистить();
	ЗагрузитьЗаказыПоставщика(ОткрытаяБаза);
	ЗагрузитьПТУ(ОткрытаяБаза);
	
	ЗагрузитьЗаказыПокупателя(ОткрытаяБаза);
	ЗагрузитьРТУ(ОткрытаяБаза);
	
	ОткрытаяБаза = Неопределено;
	
	Если ПроблемныеДокументы.Количество() > 0 Тогда
		Элементы.Страницы.ТекущаяСтраница = Элементы.Проблемы;
	КонецЕсли;
...
Процедура ЗагрузитьПТУ(ОткрытаяБаза)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Документы.ПоступлениеТоваровУслуг.НайтиПоНомеру(ДокументИсходный.Номер, ДокументИсходный.Дата)) Тогда
			// нашел
			Поступление = Документы.ПоступлениеТоваровУслуг.НайтиПоНомеру(ДокументИсходный.Номер, ДокументИсходный.Дата);
		...
		ЗаполнитьДокументПоступление(ОткрытаяБаза, Документ, ДокументИсходный);
...
&НаСервере
Процедура ЗагрузитьЗаказыПокупателя(ОткрытаяБаза)
	
	Выборка = ОткрытаяБаза.Документы.ЗаказПокупателя.Выбрать(ДатаНачала, КонецДня(ДатаОкончания));
	...
	Пока Выборка.Следующий() Цикл
		...
		СуществующийДокумент = Документы.ЗаказКлиента.НайтиПоНомеру(ДокументИсходный.Номер, ДокументИсходный.Дата);
		Если ЗначениеЗаполнено(СуществующийДокумент) Тогда
			// нашел
			Документ = СуществующийДокумент.ПолучитьОбъект();
			Документ.Записать(РежимЗаписиДокумента.ОтменаПроведения);
		...
		Документ.Заполнить(Новый Структура);
		ЗаполнитьДокументЗаказКлиента(ОткрытаяБаза, Документ, ДокументИсходный);
		ДобавитьДокументВТабличнуюЧасть(Документ);
	КонецЦикла;
...
Процедура ЗагрузитьРТУ(ОткрытаяБаза)
	...
	Выборка = ОткрытаяБаза.Документы.РеализацияТоваровУслуг.Выбрать(ДатаНачала, КонецДня(ДатаОкончания));
	...
	Пока Выборка.Следующий() Цикл
		...
		СуществующийДокумент = Документы.РеализацияТоваровУслуг.НайтиПоНомеру(ДокументИсходный.Номер, ДокументИсходный.Дата);
		Если ЗначениеЗаполнено(СуществующийДокумент) Тогда
			// нашел
			Документ = СуществующийДокумент.ПолучитьОбъект();
			Документ.Записать(РежимЗаписиДокумента.ОтменаПроведения);
		...
		Документ.Заполнить(Новый Структура);
		ЗаполнитьДокументРеализация(ОткрытаяБаза, Документ, ДокументИсходный);
		ДобавитьДокументВТабличнуюЧасть(Документ);
	КонецЦикла;
...
&НаСервере
Функция ДобавитьДокументВПроблемы(Документ, Проблема, Ошибка)
	
	НоваяСтрока = ПроблемныеДокументы.Добавить();
	НоваяСтрока.Документ = Документ.Ссылка;
...
&НаСервере
Функция ЗаказКлиента(Контрагент, СделкаИсходный)
	
	НомерИсходный = СделкаИсходный.Номер;
	ДатаИсходный = СделкаИсходный.Дата;
	Если НЕ ЗначениеЗаполнено(НомерИсходный) И НЕ ЗначениеЗаполнено(ДатаИсходный) Тогда
	...
	Запрос.УстановитьПараметр("НомерИсходный", НомерИсходный);
	Запрос.УстановитьПараметр("ДатаИсходный", ДатаИсходный);
	...
	|ГДЕ
	|	ЗаказКлиента.Номер = &НомерИсходный
	|	И ЗаказКлиента.Дата = &ДатаИсходный
...
&НаСервере
Функция ЗаказПоставщику(Контрагент, СделкаИсходный)
	...
	|ГДЕ
	|	ЗаказПоставщику.Номер = &НомерИсходный
	|	И ЗаказПоставщику.Дата = &ДатаИсходный
...
&НаСервере
Процедура ЗагрузитьРТУ2(ОткрытаяБаза)
